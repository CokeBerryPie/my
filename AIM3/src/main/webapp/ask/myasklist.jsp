						...
						<button type="button" class="tab_tit" id="1" style="left: 0px; margin-left: 217.391px;"></button>
							<div class="tab_con ty3">
								...
								<table class="tbl_list text_c">
									...
									<tbody>
										<c:forEach var="dto" items="${AskListAll }">
										<tr>
											<td>${dto.ask_bno }</td>
											<td>${dto.ask_group01 }</td>
											<td>${dto.ask_title }</td>
											<td>${dto.ask_date }</td>
										</tr>
										</c:forEach>
									...
									<ol>
										<c:if test="${totalCnt != 0}">
	  									  ...
	  									  <!-- 이전 -->
	    										...
	    										<a href="./MyAskList.ask?pageNum=${startPage-pageBlock }">[이전]</a>
	   										  </c:if>
	   									 <!-- 페이지 번호(1,2,3....) -->	  
	  										  <c:forEach var="i" begin="${startPage }" end="${endPage }" step="1">
	          									<a href="./MyAskList.ask?pageNum=${i }">${i }</a> 
	  										  </c:forEach>  
	    								 <!-- 다음 -->	
	    									  <c:if test="${endPage < pageCount }">
	       										<a href="./MyAskList.ask?pageNum=${startPage+pageBlock }">[다음]</a>
	   										 </c:if>
